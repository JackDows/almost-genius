...
export function nextReminder(status) {
  ...
  if (status.tasks) {
    const runnable = status.tasks.filter(t => t.enabled && !t.deletedAt);
    const waiting = runnable.find(t => ['pending','running','ready','failed'].includes(t.lastRun?.status) && t.lastRun.date === status.work.date && !(t.reportIncomplete && status.work.today.completed));
    if (waiting) return {time:'待完成',description:waiting.title};
    const next = runnable.filter(t => t.next).sort((a,b)=>a.next.localeCompare(b.next))[0];
    return next ? {time:new Date(next.next).toLocaleTimeString('zh-CN',{timeZone:'Asia/Shanghai',hour:'2-digit',minute:'2-digit',hour12:false}),description:next.title} : {time:'无安排',description:'可以在我的任务或聊天中创建任务'};
  }
