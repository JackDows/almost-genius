...
$taskAppNames = @('aonor-jira-reminder', 'almost-genius')
...
if ($taskExisting) {
    ...
    if ($taskArguments -match '-File "([^\"]+Run-Background\.ps1)"') {
        ...
        if ((Test-Path -LiteralPath $taskPackage) -and (Get-Content -LiteralPath $taskPackage -Raw -Encoding UTF8 | ConvertFrom-Json).name -in $taskAppNames) {
...
foreach ($taskRoot in ($taskRoots | Select-Object -Unique)) {
    ...
    if ((Get-Content -LiteralPath (Join-Path $taskRoot 'package.json') -Raw -Encoding UTF8 | ConvertFrom-Json).name -notin $taskAppNames) { throw '目标目录属于其他程序。' }
    $taskResolved = (Resolve-Path -LiteralPath $taskRoot).Path.TrimEnd('\')
    # 只终止已核对路径的本应用进程，不按通用的 node/powershell 名称终止。
    Get-CimInstance Win32_Process | Where-Object {
        ($_.ExecutablePath -eq (Join-Path $taskResolved 'desktop\bin\AlmostGenius.exe')) -or
    ...
    if (Test-Path -LiteralPath $taskRuntimeFile) {
        $taskRuntime = Get-Content -LiteralPath $taskRuntimeFile -Raw -Encoding UTF8 | ConvertFrom-Json
    ...
    if (-not $Uninstall -and $taskData -ne $taskDestination -and -not (Test-Path -LiteralPath $taskDestination)) {
        ...
        if (Test-Path -LiteralPath $taskPointer) {
            $taskGeneration = (Get-Content -LiteralPath $taskPointer -Raw -Encoding UTF8 | ConvertFrom-Json).generation
