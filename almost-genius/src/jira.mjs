...
export function upcomingJql(date, days = 2) {
  return `assignee = currentUser() AND statusCategory != Done AND duedate >= "${date}" AND duedate < "${addDays(date, days + 1)}" ORDER BY duedate ASC, key ASC`;
}
...
export class JiraClient {
  ...
  async upcoming(date = beijing().date, days = 2) {
    if (!Number.isInteger(days) || days < 0 || days > 90) throw new JiraError('临期范围必须是0至90天。');
    try {
      const issues = [];
      let startAt = 0;
      for (let page = 0; page < 100; page++) {
        const query = new URLSearchParams({ jql: upcomingJql(date, days), fields: 'summary,duedate,status', startAt: String(startAt), maxResults: '100' });
        ...
        for (const issue of data.issues) {
          ...
          if (typeof due !== 'string' || due < date || due > addDays(date, days)) continue;
