// 改编自 LifeOS Cortex，固定版本及修改说明见 THIRD_PARTY.md。
import { existsSync, lstatSync, opendirSync, readFileSync, realpathSync } from "node:fs";
import { join, relative, resolve, sep } from "node:path";
import { homedir } from "node:os";
import { createHash } from "node:crypto";
import { createCaptureEnvelope, isEnvelopeValidAt } from "./CaptureEnvelope.mjs";

export const SCHEMA = "lifeos-cortex/v1"         ;
export const EXIT = { OK: 0, INTERNAL: 1, NOT_FOUND: 3, INVALID_INPUT: 4, WRITE_REFUSED: 5 }         ;
export const LIMITS = { QUERY_CHARS: 2048, QUERY_TERMS: 64, PAGE_SIZE: 100, GRAPH_NODES: 100, GRAPH_TOKENS: 50_000, IDS: 100, WRITE_BYTES: 262_144 }         ;
// Fail-closed limits for the complete canonical corpus, checked before content reads.
export const CANONICAL_CORPUS_LIMITS = { MAX_FILES: 10_000, MAX_FILE_BYTES: 8 * 1024 * 1024, MAX_TOTAL_BYTES: 128 * 1024 * 1024, MAX_RECORDS: 50_000 }         ;
...
export function validateCortexEnvelope(value         )           {
  const errors           = [];
...
function envelope(command        , data         )                 { return { schema: SCHEMA, ok: true, command, data, error: null }; }
function failure(command        , code        , message        )                 { return { schema: SCHEMA, ok: false, command, data: null, error: { code, message } }; }
function ok(command        , data         )                  { return checked({ exitCode: EXIT.OK, envelope: envelope(command, data) }); }
function fail(command        , exitCode        , code        , message        )                  { return checked({ exitCode, envelope: failure(command, code, message) }); }

class IntegrityError extends Error {}
function integrity(message        )        { throw new IntegrityError(message); }
function isWithin(root        , path        )          { return path === root || path.startsWith(root + sep); }
function checkedRealpath(path        , rootReal        )         {
...
function filesUnder(root        , corpusRoot        )           {
...
function fieldsFromMarkdown(content        )                         {
...
function validateRecord(record              )               {
  ...
  for (const [name, value] of [["created", record.created], ["updated", record.updated]]         ) if (!validTimestamp(value)) integrity(`Invalid ${name} timestamp for ${record.id}: ${value}`);
  for (const [name, value] of [["valid_from", record.valid_from], ["valid_until", record.valid_until]]         ) if (value != null && !validTimestamp(value)) integrity(`Invalid ${name} timestamp for ${record.id}: ${value}`);
...
function sanitizeContent(content        , fields                        )         {
...
function parseMarkdown(path        , root        )               {
...
function pinCanonicalRoot(root        )         {
...
function canonicalFiles(root        )           {
  ...
  if (existsSync(knowledge)) {
    const hot           = [], opened = opendirSync(root);
    try {
...
export function enumerateCanonicalCorpus(root        )                                                             {
  const canonicalRoot = pinCanonicalRoot(root);
  const files = canonicalFiles(canonicalRoot), records                 = [], ids = new Map                ();
  if (files.length > CANONICAL_CORPUS_LIMITS.MAX_FILES) integrity(`Canonical corpus exceeds file count limit (${CANONICAL_CORPUS_LIMITS.MAX_FILES})`);
  let totalBytes = 0;
  ...
  for (const path of files) {
    if (path.endsWith(".md")) addRecord(parseMarkdown(path, canonicalRoot));
      ...
      for (const key of ["id","type","title","created","updated","content"]         ) if (typeof row[key] !== "string" || !row[key].trim()) integrity(`JSONL ${key} must be a non-empty string at ${location}`);
      for (const key of ["source_kind","source"]         ) if (row[key] !== undefined && (typeof row[key] !== "string" || !row[key].trim())) integrity(`JSONL ${key} must be a non-empty string at ${location}`);
      ...
      if (row.related !== undefined && (!Array.isArray(row.related) || row.related.some((id         ) => typeof id !== "string" || !id.trim()))) integrity(`JSONL related must contain only non-empty string IDs at ${location}`);
...
export function loadCanonicalRecords(root        )                 { return enumerateCanonicalCorpus(root).records; }
...
function parseArguments(args          )                  {
  ...
  const options = new Map                       (), positionals           = [];
...
function opt(parsed        , name        )                     { const value = parsed.options.get(name); return typeof value === "string" ? value : undefined; }
function positiveInt(value                    , fallback        , max = Number.MAX_SAFE_INTEGER)                { if (value === undefined) return fallback; const n = Number(value); return Number.isInteger(n) && n > 0 && n <= max ? n : null; }
function nonNegativeInt(value                    , fallback        )                { if (value === undefined) return fallback; const n = Number(value); return Number.isInteger(n) && n >= 0 && n <= LIMITS.PAGE_SIZE ? n : null; }
export function toCortexCard(record              , score        )             { return { id: record.id, type: record.type, created: record.created, updated: record.updated, provenance: record.provenance, score: Number(score.toFixed(6)), est_tokens: Math.ceil(record.content.length / 4) }; }
function terms(text        )           { return [...new Intl.Segmenter("zh", {granularity:"word"}).segment(text.toLowerCase())].filter(part=>part.isWordLike).map(part=>part.segment); }
export function rankBM25(records                , query        )                                                 {
  ...
  return records.map((record) => {
    const words = terms(`${record.title} ${record.content}`), frequencies = new Map                ();
...
export function activeCortexCards(records                , now = new Date())               { return activeCortexRecords(records, now).map(record => toCortexCard(record, 0)); }
export function activeCortexRecords(records                , now = new Date())                 { return records.filter((record) => isEnvelopeValidAt({ source: record.provenance.source, channel: "canonical", timestamps: { captured_at: record.created }, valid_from: record.valid_from, valid_until: record.valid_until, session_id: record.provenance.session, content: record.content }, now)); }
function filtered(records                , parsed        )                        {
...
function paged(items              , parsed        ) { const page=positiveInt(opt(parsed,"--page"),1), size=positiveInt(opt(parsed,"--page-size"),10,LIMITS.PAGE_SIZE); return page===null||size===null?null:{items:items.slice((page-1)*size,page*size),total:items.length,page,page_size:size}; }
function expandGraph(records                , startId        , maxNodes        , maxTokens        ) { const byId=new Map(records.map(r=>[r.id,r])); const start=byId.get(startId); if(!start)return null; const queue=[start],seen=new Set        (),items             =[];let est=0;while(queue.length&&items.length<maxNodes){const current=queue.shift() ;if(seen.has(current.id))continue;seen.add(current.id);const next=toCortexCard(current,0);if(est+next.est_tokens>maxTokens)break;items.push(next);est+=next.est_tokens;for(const id of current.related){const linked=byId.get(id);if(linked&&!seen.has(id))queue.push(linked);}}return{items,est_tokens:est,truncated:queue.length>0}; }
export function canonicalCorpusDigest(records                )         { const normalized=records.slice().sort((a,b)=>a.id.localeCompare(b.id)).map(r=>({id:r.id,type:r.type,title:r.title,created:r.created,updated:r.updated,provenance:r.provenance,content:r.content,related:r.related,valid_from:r.valid_from??null,valid_until:r.valid_until??null})); return createHash("sha256").update(JSON.stringify(normalized)).digest("hex"); }

export async function runCortex(args          , runtime                = {})                           {
  const command = args[0] ?? "";
  ...
  if ((command === "remember" || command === "propose")) {
    ...
    let item    ;try{item=JSON.parse(raw);}catch{return fail(command,EXIT.INVALID_INPUT,"invalid_input","Write payload must be valid JSON");}
    ...
    const memoryAdd=runtime.memoryAdd??(()=>({ok:false,message:"Almost Genius requires an explicit memory writer"})); const result=memoryAdd(item);
  ...
  let memoryRoot=requestedRoot, records               ;try{const corpus=enumerateCanonicalCorpus(requestedRoot);memoryRoot=corpus.root;records=corpus.records;}catch(error){if(error instanceof IntegrityError)return fail(command,EXIT.INVALID_INPUT,"integrity_error",error.message);return fail(command,EXIT.INTERNAL,"internal_error",error instanceof Error?error.message:String(error));}
  ...
  const byId=new Map(validRecords.map(r=>[r.id,r]));const chosen=parsed.positionals.map(id=>byId.get(id)).filter((r)                  =>Boolean(r));if(chosen.length!==parsed.positionals.length)return fail(command,EXIT.NOT_FOUND,"not_found","One or more records were not found");return ok(command,{format:"lifeos-cortex-export/v1",items:chosen});
}
