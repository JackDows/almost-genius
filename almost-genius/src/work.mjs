...
export class WorkService {
  ...
  async setReminder(time) {
    ...
    await this.store.update(state => {
      ...
      if (state.tasksSchema && (!state.tasks['daily-report']?.enabled || state.tasks['daily-report'].deletedAt || Date.parse(state.tasks['daily-report'].pauseUntil) > now)) throw new WorkError('日报任务已暂停或删除，请先恢复日报任务。');
  ...
  async flushOutbox() {
    ...
    try { for (const [kind, message] of pending) {
      if (beijing(this.now()).date !== date) break;
      try {
        await this.wecom.push(message.text, { completedChunks: message.wecomChunks || 0, onProgress: count => this.store.update(s => { const current = dayRecord(s, date).outbox?.[kind]; if (current?.id === message.id) current.wecomChunks = count; }) });
