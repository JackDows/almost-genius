...
$taskExecutable = Join-Path $PSScriptRoot 'bin\JiraReminder.exe'
$taskIcon = Join-Path $PSScriptRoot 'bin\WorkReminder.ico'
...
foreach ($taskDirectory in @([Environment]::GetFolderPath('DesktopDirectory'),[Environment]::GetFolderPath('Programs'))) {
    $taskShortcutPath = Join-Path $taskDirectory 'Jira 工作提醒.lnk'
...
Write-Output '已创建桌面及开始菜单快捷方式：Jira 工作提醒。'
