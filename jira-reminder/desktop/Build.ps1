...
$taskIconPath = Join-Path $taskOutput 'WorkReminder.ico'
...
$taskExecutable = Join-Path $taskOutput 'JiraReminder.exe'
...
Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'JiraReminder.exe.config') -Destination $taskOutput -Force
