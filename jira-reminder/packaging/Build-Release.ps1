...
try {
    ...
    foreach ($taskName in @('src','web','node_modules')) { Copy-Item -LiteralPath (Join-Path $taskRoot $taskName) -Destination $taskStage -Recurse }
    foreach ($taskName in @('package.json','package-lock.json','README.md','Paths.ps1','Start.ps1','Run-Background.ps1','Install-Autostart.ps1','Remove-Autostart.ps1')) { Copy-Item -LiteralPath (Join-Path $taskRoot $taskName) -Destination $taskStage }
    ...
    $taskArtifact = Join-Path $taskRoot ('dist\JiraWorkReminder-'+$taskVersion+'-Setup-x64.exe')
