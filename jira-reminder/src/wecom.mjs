...
export class WecomSetup {
  ...
  async push(content) {
    ...
    for (const line of content.split('\n')) {
      if (Buffer.byteLength(current + '\n' + line) > 3500 && current) { chunks.push(current); current = ''; }
      current += (current ? '\n' : '') + line;
    }
    if (current) chunks.push(current);
    for (const chunk of chunks) await this.client.sendMessage(this.credentials.userId, { msgtype: 'markdown', markdown: { content: chunk } });
    this.lastPushAt = new Date().toISOString();
  }
